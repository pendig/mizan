...
json_has_model() {
  local payload="$1"
  local model="$2"
  printf '%s' "${payload}" | python3 - "$model" <<'PY'
import json,sys
model=sys.argv[1]
...
wait_for_model() {
  ...
  for _ in $(seq 1 "${WAIT_SECONDS}"); do
    local models
    models="$(curl -fsS "$url" -H "authorization: Bearer ${api_key}")"
...
assert_usage_has_rows() {
  ...
  printf '%s' "${payload}" | python3 - "$require_daemon_node_id" <<'PY'
import json,sys
...
admin_login_json="$(curl -fsS -X POST "${BASE_URL}/auth/login" \
  -H 'content-type: application/json' \
  -d "{\"email\":\"${ADMIN_EMAIL}\",\"password\":\"${ADMIN_PASSWORD}\"}")"
admin_session_token="$(printf '%s' "${admin_login_json}" | json_field access_token)"

admin_api_key_response="$(curl -fsS -X POST "${BASE_URL}/api-keys" \
  -H "authorization: Bearer ${admin_session_token}" \
  -H 'content-type: application/json' \
  -d '{"label":"distributed-smoke-admin"}')"
admin_api_key="$(printf '%s' "${admin_api_key_response}" | json_field key)"

echo "Creating daemon node"
daemon_node_json="$(curl -fsS -X POST "${BASE_URL}/admin/daemon-nodes" \
  -H "authorization: Bearer ${admin_api_key}" \
  -H 'content-type: application/json' \
  -d '{"label":"distributed-smoke-node","hostname":"localhost"}')"
...
if ! admin_models="$(wait_for_model "${admin_api_key}" "${BASE_URL}/v1/models" "${MODEL_NAME}")"; then
...
user_login_json="$(curl -fsS -X POST "${BASE_URL}/auth/login" \
  -H 'content-type: application/json' \
  -d "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\"}")"
user_session_token="$(printf '%s' "${user_login_json}" | json_field access_token)"

user_api_key_json="$(curl -fsS -X POST "${BASE_URL}/api-keys" \
  -H "authorization: Bearer ${user_session_token}" \
  -H 'content-type: application/json' \
  -d '{"label":"distributed-smoke-user"}')"
...
curl -fsS -X POST "${BASE_URL}/v1/chat/completions" \
  ...
  -H 'content-type: application/json' \
  -d "{\"model\":\"${MODEL_NAME}\",\"messages\":[{\"role\":\"user\",\"content\":\"hello from mizan distributed smoke\"}],\"max_tokens\":32}" >/dev/null

echo "Verifying usage and metrics"
user_usage_json="$(curl -fsS "${BASE_URL}/v1/usage" -H "authorization: Bearer ${user_api_key}")"
assert_usage_has_rows "${user_usage_json}" 0
admin_usage_json="$(curl -fsS "${BASE_URL}/admin/usage" -H "authorization: Bearer ${admin_api_key}")"
